...
% Analyse individuals using logNormal: use both weighted least squares and
% maximimum likelihood by fitting both full and constrained
% (i.e., no interaction) lognormal regression models
%
...
n = 4;    % Number of subjects (small n)
M = 500;  % Data points per subject
nBootstrap = 1000;   % Number of bootstrap repetitions
nbootsamples = 400;  % Number of samples per bootstrap (downsampling)
N = 100; % Number of simulation iterations (on each iteration a new set of subjects is generated)

pset = [.1 .25 .5 .75 .9]; % Probability of interaction (for each subject)
...
%% Lognormal models for maximum likelihood parameter estimation (negative log likelihoods)
...
for repidx = 1:N
    ...
    for bidx = 1:nBootstrap
        ...
        if mod(bidx, 100) == 0
            ...
            time = toc;
            displayTime(time);
            save simulationRun205
        end
    end
    
    % Save output
    rboot = reshape(bootout', 5, 4, size(bootout, 1));
    meanWlsEstimate = mean(rboot(2,:,:), 3);
    stdWlsEstimate  = std(rboot(2,:,:), [], 3);
    pCountWls       = mean(reshape(rboot(3,:,:), 4, nBootstrap)' <= .05);
    meanFitEstimate = mean(rboot(4,:,:), 3);
    stdFitEstimate  = std(rboot(4,:,:), [], 3);
    pCountFit       = mean(reshape(rboot(5,:,:), 4, nBootstrap)' <= .05);
    boSummary = [(1:4)', meanWlsEstimate', stdWlsEstimate', pCountWls', meanFitEstimate', stdFitEstimate', pCountFit'];
    
    bo(repidx,:) = reshape(boSummary', numel(boSummary), 1)';
    go(repidx,:) = mean(groupout(:,4) <= .05);
end
save smamllNsimulation